...
export default function Home() {
  ...
  const getCurrentDate = () => {
    ...
    return dateStr.replace(/(^\w)/, (match) => match.toUpperCase()).replace(/ de /g, ' De ')
  }
  ...
  const handleOpenSubmit = async (e: React.FormEvent) => {
    ...
    const numericAmount = openingAmount
    ...
    return (
      <div className="home-wrapper">
        <h2 style={{ color: 'white' }}>
          Error: CashRegisterContext no encontrado. Revisa tu App.tsx
        </h2>
      </div>
    )
  }
          ...
          <form onSubmit={handleOpenSubmit} className="open-register-form">
            ...
            <div className="input-container">
              ...
              <input
                ...
                onChange={(e) => setOpeningAmount(parseInputValue(e.target.value))}
                required
              />
            </div>
            ...
            {errorMsg && (
              <p className="error-text" style={{ color: 'red', fontSize: '0.8rem' }}>
                {errorMsg}
              </p>
            )}
