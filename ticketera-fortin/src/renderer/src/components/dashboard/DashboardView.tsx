...
const DashboardContent = ({ onOpenModal }: { onOpenModal: () => void }) => {
  ...
  return (
    <div style={{ width: '100%', height: 'calc(100vh - 180px)', overflowY: 'auto', paddingTop: '80px', position: 'relative' }}>
      ...
      <div className="dashboard-container" style={{ padding: '20px', gap: '20px', display: 'flex', flexDirection: 'column', maxWidth: '900px', margin: '0 auto', paddingBottom: '120px' }}>
        
        <DashboardHeader onOpenModal={onOpenModal} />

        {isLoading ? (
          <p style={{ textAlign: 'center', marginTop: '50px', color: 'gray', fontWeight: 'bold' }}>Cargando datos...</p>
        ) : (
          <>
            {boxesToShow.length === 0 ? (
               <p style={{ textAlign: 'center', marginTop: '50px', color: 'gray' }}>
                 {!isFiltering ? 'No hay ninguna caja abierta actualmente.' : 'No se encontraron cajas en las fechas seleccionadas.'}
               </p>
            ) : (
              boxesToShow.map((box) => (
